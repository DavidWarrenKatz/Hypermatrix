...
from config import methy_directory, filtered_list, chrom_file, resolutions, output_directory, mappability_threshold, normalization, iterations, reference_genome
...
bulk_data_input_file = f'{output_directory}/nan_removed_bulk_data_resolution_res{resolution}.pkl'
with open(bulk_data_input_file, 'rb') as f:
    bulk_data = pickle.load(f)
print(f"bulk_data loaded from {bulk_data_input_file}")

# Load original_bulk_data from the file
original_bulk_data_input_file = f'{output_directory}/original_bulk_data_res{resolution}.pkl'
...
chromosome_results_input_file = f'{output_directory}/chromosome_results_res{resolution}.pkl'
...
plot_eigenvectors_with_heatmap(
    ...
    save_path=f"{output_directory}/../../files/AB_compartment_heatmap_ch{chr}_example_iter{iterations}.png"
)
