...
public class BwaMem	
  ...
  public void updateMoreScoringParameters(final int threads, final int alignScore, final int softClipForSuppl,
          final int clipPen5, final int clipPen3){
	  update_more_parameters( threads,  alignScore, softClipForSuppl,clipPen5, clipPen3);
  }
	...
	public String[] align(final List<ShortRead> ks) throws IOException
	{
	if(ref==0L) return null;
	if(ks==null) throw new IllegalArgumentException("ks1 is null");

	return align(
		ks.toArray(new ShortRead[ks.size()])
		);
	}
	...
	public String[] align(final ShortRead ks[]) throws IOException
	{
	if(ref==0L) return null;
	if(ks==null) throw new IllegalArgumentException("ks1 is null");
	
	if(ks.length==0) return null;
	return alignR(this.bwaIndex,ks);
	}
  ...
  private static native long mem_opt_init();
	...
	private native void update_more_parameters(int threads,  int alignScore,  int softClipForSuppl,int clipPen5,  int clipPen3);
	...
	private native String[] alignR(BwaIndex bwaIndex,final ShortRead ks[])  throws IOException;
	}
