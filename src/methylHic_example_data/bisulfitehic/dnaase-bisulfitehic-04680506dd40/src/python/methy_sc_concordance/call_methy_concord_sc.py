import sys, glob, gzip, subprocess, re, os
import numpy as np
#import methy_sc_concordance.poibin.PoiBin
import collections
import numpy as np


def call_methy_concordance(argv):
    input_dir = argv[0]  # /home/gul030/yaping_analysis/gul030/yaping_results/sc_mapping_test/analysis_108_cell_naive_2i/final_set_108_cells/naive/JL_*.mm9.mapQ30.chr19.allele*_methy.bw
    input_reg = argv[1]
    output = argv[2]
    # iterate each 500bp window
    window_val = {}
    window_count_val = {}
    os.chdir(input_dir)
    for file in glob.glob(input_reg):
        with open(file, 'r') as f:
            for line in f:
                line = line.rstrip('\n')
                tmp_array = line.split("\t")
                name = tmp_array[0]
                if int(tmp_array[2]) > 0:
                    if name not in window_val:
                        window_val[name] = [tmp_array[5]]
                        window_count_val[name] = [tmp_array[3]]
                    else:
                        window_val[name].append(tmp_array[5])
                        window_count_val[name].append(tmp_array[3])

    # build the pbniom distribution for each window with at least 3 cells have value
    print("building distribution...")
    poibin_collections = {}
    window_count_collections = {}
    for key, value in window_val.items():
        if (value is not None) and len(value) > 10:
            #print(np.array(value,dtype='float32'))
            pb = PoiBin(np.array(value,dtype='float32'))
            poibin_collections[key] = pb
            window_count_collections[key] = np.sum(np.array(window_count_val[key], dtype='int32'))
    print("output...")
    with gzip.open(output, 'wb') as out:
        for key1, poibin in poibin_collections.items():
            window_point_num = len(window_val[key1])
            w1_chr, w1_start, w1_end = unwrap_interval(key1)
            for key2, methy_count in window_count_collections.items():
                if methy_count < window_point_num:
                    pvals = poibin.pval(methy_count)
                    w2_chr, w2_start, w2_end = unwrap_interval(key2)
                    name = w1_chr + ":" + w1_start + ":" + w1_end + ":" + w2_start + ":" + w2_end
                    line = w1_chr + "\t" + str(w1_start) + "\t" + str(w1_end) + "\t" + \
                              w2_chr + "\t" + str(w2_start) + "\t" + str(w2_end) + "\t" + \
                              name + "\t" + str(pvals) + "\n"
                    out.write(line.encode('utf-8'))


def unwrap_interval(interval):
    m = re.search('(chr\d+):(\d+)-(\d+)', interval)
    chrom = m.group(1)
    start = m.group(2)
    end = m.group(3)
...
# due to the error of import pyBigWig and bx-python in mac OSX
def wrap_bigwig_tool(chrom, start, end):
    result = subprocess.run(['ls', (chrom + " " + str(start) + " " + str(end) + " " + str(1))], stdout=subprocess.PIPE)
...
if __name__ == "__main__":
    call_methy_concordance(sys.argv[1:])
