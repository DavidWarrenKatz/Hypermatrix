...
/** BWAMem : update part of mem_opt scoring parameters  */
JNIEXPORT void JNICALL QUALIFIEDMETHOD(BwaMem_update_1more_1parameters)(JNIEnv *env, jobject self,
        jint t,
        jint S, jint Y,
        jint L5, jint L3)
  {
 ...
 * Signature: ([Lcom/github/lindenb/jbwa/jni/ShortRead;[Lcom/github/lindenb/jbwa/jni/ShortRead;)I
 */
JNIEXPORT jobjectArray JNICALL QUALIFIEDMETHOD(BwaMem_alignR)(
	JNIEnv *env,
	 jobject self,
	 jobject bwaIndex,
	 jobjectArray ks
	...
	bwaidx_t* idx= _getBwaIndex(env,bwaIndex);
  	...
  	numPairs=(*env)->GetArrayLength(env,ks);


  	seqs=calloc(numPairs,sizeof(bseq1_t));
  	if(seqs==NULL) return NULL;
  	...
  	for( i = 0; i < numPairs; i++)
		{
			...
			jsize length;
			jobject readName;
			bseq1_t* curr_bseq1=&seqs[i];

			jobject readObj = (*env)->GetObjectArrayElement(env,ks, i);
	...
	samRetArray = (jobjectArray)(*env)->NewObjectArray(env, (numPairs ),
         	(*env)->FindClass(env,"java/lang/String"),
         	NULL
         	);
	VERIFY_NOT_NULL(samRetArray);
